...
| `settings` denies `Read`/`Grep`/`Glob` identically on `/proc`, `/sys`, `/home/runner/work/_temp`, and `/home/runner/.config` — the on-disk locations of the OAuth token — while the checkout stays readable | Done |
...
- Claude reads the PR and may run named git verbs plus project lint on the checked-out PR head. Those lint scripts are PR-controlled. The token is not in the job env (`inject-env-vars: false`) and is passed only through the action input, never written to `$GITHUB_OUTPUT` or `$GITHUB_ENV`, and the `settings` deny rules keep `Read`/`Grep`/`Glob` out of `/proc`, `/sys`, `/home/runner/work/_temp`, and `/home/runner/.config`, so a prompt-injected read cannot reach the Claude process environment. `CLAUDE_CODE_SUBPROCESS_ENV_SCRUB=1` strips credential env vars from Bash subprocesses, but it needs `bubblewrap` on the runner to create the PID namespace and the runner image does not ship it — treat the deny rules and the absence of write tools as the load-bearing controls, not subprocess isolation. A malicious lint script still runs, and a malicious same-repo PR can still prompt-inject. **Residual risk — state it plainly.** Push is blocked by `contents: read` (checkout also uses `persist-credentials: false`). It cannot merge.
