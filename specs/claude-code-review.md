...
| `settings` denies `Read`/`Grep`/`Glob` outside the checkout (`/proc`, `/sys`, `/home/runner`), so unscoped `Read` cannot reach the Claude process environment | Done |
